...
-- Maximum amount of stored chat messages
local CHATLOG_LIMIT = 200
-- Maximum amount of entries on one page
local CHATLOG_PER_PAGE = 25
-- Show the unread messages notification
local CHATLOG_SHOW_UNREAD_MESSAGES = true
...
local TIMESTAMP_HEADER = Node("div", {class = "text-center"}, Icon("clock"))
...
AddEventHandler("chatMessage", function(source, author, message)
    ...
    while #CHATLOG_HISTORY > CHATLOG_LIMIT do
        table.remove(CHATLOG_HISTORY, #CHATLOG_HISTORY)
    end
end)
for i = 1, CHATLOG_LIMIT do
    CHATLOG_TALLY = CHATLOG_TALLY + 1
    table.insert(CHATLOG_HISTORY, 1, {0, "glitchdetector", "sample message", os.time() + i})
end
...
exports['webadmin']:registerPluginPage(PAGE_NAME, function(data)
    ...
    return Nodes({
        ...
        FAQ.PageTitle({PAGE_TITLE, " ", Badge("info", CHATLOG_TALLY)}),
